...
def get_mongo_client(host, port, username, password, authdb):
    if username and password:
        uri = f"mongodb://{username}:{password}@{host}:{port}/?authSource={authdb}"
    else:
        uri = f"mongodb://{host}:{port}/"
    return MongoClient(uri)

def get_repl_set_member_info(repl_client):
    try:
...
def get_instances_info(client):
    try:
        primary_host = client.admin.command('isMaster')['primary']
        repl_client = MongoClient(primary_host)
        return get_repl_set_member_info(repl_client)
    except Exception as e:
...
def get_oplog_info(client):
    try:
        ...
        oplog_stats = client.admin.command('collStats', 'oplog.rs')
        ...
        last_election_date = client.admin.command('replSetGetStatus')['members'][0]['electionDate']
        ...
        for db_name in db_names:
            ...
            for coll_name in collection_names:
                try:
                    ...
                    continue  # Skip views and other non-collection namespaces
...
def print_report(client):
    ...
    report = {
        ...
        "Datadir": server_status['storageEngine'].get('dbPath', 'N/A'),
...
def main():
    parser = argparse.ArgumentParser(description="MongoDB Replicated Cluster Info Retriever")
